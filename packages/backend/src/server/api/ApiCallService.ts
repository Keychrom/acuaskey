...
export class ApiCallService implements OnApplicationShutdown {
	...
	#sendApiError(reply: FastifyReply, err: ApiError): void {
		...
		if (err.httpStatusCode === 401) {
			reply.header('WWW-Authenticate', 'Bearer realm="Acuaskey"');
			...
			reply.header('WWW-Authenticate', `Bearer realm="Acuaskey", error="invalid_request", error_description="${err.message}"`);
			statusCode = statusCode ?? 400;
		} else if (err.kind === 'permission') {
			// (ROLE_PERMISSION_DENIEDは関係ない)
			if (err.code === 'PERMISSION_DENIED') {
				reply.header('WWW-Authenticate', `Bearer realm="Acuaskey", error="insufficient_scope", error_description="${err.message}"`);
			}
	...
	#sendAuthenticationError(reply: FastifyReply, err: unknown): void {
		if (err instanceof AuthenticationError) {
			...
			reply.header('WWW-Authenticate', `Bearer realm="Acuaskey", error="invalid_token", error_description="${message}"`);
