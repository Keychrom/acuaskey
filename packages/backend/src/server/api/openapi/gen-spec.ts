...
export function genOpenapiSpec(config: Config, includeSelfRef = false) {
	const spec = {
		...
		externalDocs: {
			...
			url: 'https://github.com/kokonect-link/Acuaskey',
		},
	...
	for (const endpoint of copiedEndpoints) {
		...
		if (endpoint.meta.secure) {
			desc += '**Internal Endpoint**: This endpoint is an API for the Acuaskey mainframe and is not intended for use by third parties.\n';
		}
		...
		const info = {
			...
			externalDocs: {
				...
				url: `https://github.com/kokonect-link/Acuaskey/blob/develop/packages/backend/src/server/api/endpoints/${endpoint.name}.ts`,
			},
			...(endpoint.meta.tags ? {
